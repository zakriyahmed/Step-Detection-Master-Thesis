            ...
            for filtr in filter_settings:
                for justHand in sensor_settings:
                    ...
                    # Create an instance of the LSTM network
                            model = am.CNN2h(ws,channel,(1,channel,ws)).cuda()
                            ...
                            model,_ =  train.trainCNN2h(model,dataloader,ws0,we0,ws1,we1,epochs=20,lr=0.0001)

                            testpredictionwoc = utils.get_prediction(model.eval().cpu(),TestData[test_here:test_there],ws,channel)
